(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('movingsystemsApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-movingsystemsApp-error');
                var entityKey = httpResponse.headers('X-movingsystemsApp-params');
                if (errorHeader) {
